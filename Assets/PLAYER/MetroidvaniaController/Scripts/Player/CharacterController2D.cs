...
public class CharacterController2D : MonoBehaviour
{
	...
	{
		if (canMove) {
			if (dash && canDash && !isWallSliding && PlayerSkills.Instance.canDash)
						{
							// Só inicia a corrotina se tiver a habilidade
			...
			else if (!m_Grounded && jump && canDoubleJump && !isWallSliding && PlayerSkills.Instance.canDoubleJump)
			{
				canDoubleJump = false;
			...
			{
				// AQUI ESTÁ A MUDANÇA: Adicione && PlayerSkills.Instance.canWallClimb
				// Se não tiver a habilidade, ele nunca entra no modo WallSlide
				if ((!oldWallSlidding && m_Rigidbody2D.linearVelocity.y < 0 || isDashing) && PlayerSkills.Instance.canWallClimb)
				{
					isWallSliding = true;
